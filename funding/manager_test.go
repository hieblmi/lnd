...
func openChannel(t *testing.T, alice, bob *testNode, localFundingAmt,
	...
	*wire.OutPoint, *wire.MsgTx) {
	...
	publ := fundChannel(
		t, alice, bob, localFundingAmt, pushAmt, false, false, 0, 0,
		numConfs, updateChan, announceChan, nil,
	)
...
func fundChannel(t *testing.T, alice, bob *testNode, localFundingAmt,
	pushAmt btcutil.Amount, subtractFees, fundMax bool, fundUpToMaxAmt,
...
func TestFundingManagerFundAll(t *testing.T) {
	...
	for _, test := range tests {
		...
		fundingTx := fundChannel(
			t, alice, bob, test.spendAmt, pushAmt, true, false, 0,
			0, 1, updateChan, true, nil,
		)
...
func TestFundingManagerFundMax(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// We are using the entirety of two inputs
			// for the funding of a channel, hence expect
			// no change output.
			coins: constructTestUtxos(
		...
		{
			// We are using a fraction of two inputs for
			// the funding of our channel, hence expect
			// a change output.
			coins: constructTestUtxos(
				MaxBtcFundingAmount/2, MaxBtcFundingAmount/2,
			),
		...
		{
			// We are funding a channel with half of
			// the balance in our wallet hence expect
			// a change output. Furthermore we push
			// half of the funding amount to the
			// remote end which we expect to succeed.
			coins:          constructTestUtxos(MaxBtcFundingAmount),
			fundUpToMaxAmt: MaxBtcFundingAmount / 2,
			fundMinAmt:     MaxBtcFundingAmount / 4,
	...
	for _, test := range tests {
		...
		fundingTx := fundChannel(
			t, alice, bob, 0, pushAmt, false, true,
			test.fundUpToMaxAmt, test.fundMinAmt, 1, updateChan,
			true, nil,
		)
...
func TestFundingManagerZeroConf(t *testing.T) {
	...
	fundingTx := fundChannel(
		t, alice, bob, fundingAmt, pushAmt, false, false, 0, 0, 1,
		updateChan, true, &channelType,
	)
...
// inconsisten bahvior would have resulted when specifying fundmax for different
// types of channel openings.
