...
func TestCalculateFees(t *testing.T) {
	...
	for _, test := range testCases {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.Equal(
				t, test.expectedFeeWithChange, feeWithChange,
			)
		})
	}
}
...
func TestCoinSelect(t *testing.T) {
	...
	testCases := []testCase{
		...
		},
		{
			...
			outputValue: 1*coin - fundingFee(feeRate, 1, false) -
				dustLimit,
	...
	for _, test := range testCases {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			// If we got an expected error, there is nothing more
			// to test.
			if test.expectErr {
				return
			}
...
func TestCoinSelectSubtractFees(t *testing.T) {
	...
	testCases := []testCase{
		...
		},
		{
			...
			expectErr: "output amount(<amt> BTC) after " +
				"subtracting fees(<amt> BTC) below dust " +
				"limit(<amt> BTC)",
		},
		{
		...
		},
		{
			...
			spendValue: fundingFee(feeRate, 1, false) +
				dustLimit + 1,
	...
	for _, test := range testCases {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err != nil {
				...
				case test.expectErr != removeAmounts(
					err.Error(),
				):
